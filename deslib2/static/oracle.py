...
from deslib2.static.base import BaseStaticEnsemble
...
class Oracle(BaseStaticEnsemble):
    ...
    #         Class labels of each sample in X.
    #
    #     pred_test : array-like of shape (n_samples, n_classifiers)
    #         Predicted labels for each sample in X by each base classifier.
    #
    #     Returns
    #     -------
    #     predicted_labels : array of shape (n_samples)
    ...
    #
    #     y = self.enc_.transform(y)
    #     idx_sel_classifier = np.argmax(pred_test, axis=1)
    #     predicted_labels = pred_test[np.arange(pred_test.shape[0]), idx_sel_classifier]
    #
    #     return self.classes_.take(predicted_labels.astype(int))
