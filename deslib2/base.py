...
from deslib2.util import KNNE
from deslib2.util import faiss_knn_wrapper
from deslib2.util.dfp import frienemy_pruning_preprocessed
from deslib2.util.instance_hardness import hardness_region_competence
...
class BaseDS(BaseEstimator, ClassifierMixin):
    ...
    def obtain_best_classifiers(self, X, y, probs=None):
        ...
        Returns
        -------
        selected_algorithms : array of shape (n_samples, n_algorithms)
        ...
        selected_algorithms = np.empty([X.shape[0], len(self.algorithms_pool)], dtype=object)
        need_proba = self.needs_proba or self.voting == 'soft'
        
        #serve para não cair na condição de haver acordos (ind_all_agree), visando forçar a passagem pela seleção dinamica
        #necessario apenas caso não tivesse as predições dos algoritos para os exemplos de teste
        #base_preds = np.empty([X.shape[0], len(self.algorithms_pool)])
        #base_preds[np.arange(0,X.shape[0])] = np.arange(len(self.algorithms_pool),dtype=np.intp)
        #print(base_preds)
        base_preds, base_probas = self._preprocess_predictions(X, need_proba)
        base_preds = y
        #base_probas = probs
        ...
        ind_disagreement, ind_all_agree = self._split_agreement(base_preds)
        if ind_all_agree.size:
            #print("All agree: ", ind_all_agree.size)
            preds[ind_all_agree] = base_preds[ind_all_agree, 0]
            
            # ind_all_agree são os indices de X_test em que todos os algoritmos concordam, ou seja, todos deve ser selecionados
            selected_algorithms[ind_all_agree] = self.algorithms_pool

        # predict with IH
        if ind_disagreement.size:
            #print("Dynamic selection: ", ind_disagreement.size)
            distances, ind_ds_classifier, neighbors = self._IH_prediction(
                X, ind_disagreement, preds, is_proba=False
            )
            ...
            if ind_ds_classifier.size:
                ...
                preds[inds] = preds_ds
                temp = self.select_classifiers(sel_preds, neighbors, distances)
    ...
    def show_selected_classifiers(self, classifiers):
        ...
        for i in range(len(classifiers)):
            clfs = []
            for j in range(len(self.algorithms_pool)):
                if(classifiers[i][j] == True):
                    clfs.append(self.algorithms_pool[j])
                else:
                    clfs.append("")                
            result.append(clfs)
        return result
    ...
    def _fit_pool_classifiers(self, X, y):
        ...
        self.pool_classifiers_ = BaggingClassifier(
            random_state=self.random_state_, n_jobs=self.n_jobs, n_estimators=len(self.algorithms_pool))
