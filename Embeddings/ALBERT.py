...
model = AlbertModel.from_pretrained(MODEL_NAME, output_hidden_states=True).to(device)
...
def extract_features(text):
    ...
    last_tokens = outputs.last_hidden_state[:, -4:, :]
    return last_tokens.mean(dim=1)

def run_batch(dataset_name, split):
    df_path = f"data/{dataset_name}_{split}.csv"
    out_path = f"outputs/ALBERT_{dataset_name}_{split}.npz"

    df = pd.read_csv(df_path)
    df = df.dropna(subset=[TEXT_COLUMN])
    texts = df[TEXT_COLUMN].astype(str).tolist()

    print(f">>> Processing: {dataset_name}_{split} with ALBERT | Samples: {len(texts)}")
